...
Song buildDemoSong()
{
    ...
    // Each generator's pattern goes in its first slot, so the demo opens with
    // A1 lit in the slot grid rather than sitting outside it. The slot only
    // supplies a default name, so the descriptive names below still stand.
    const PatternSlot slotA1 { 0, 0 };

    // Bass: 4-beat pattern, played through the whole song (beats 0..16)
    {
        auto bass = song.addGenerator ("Bass", "internal-synth", nullptr);
        auto pattern = bass.addPattern (slotA1, "Bassline", 4.0, nullptr);
    ...
    // Chords: 8-beat pattern (Am -> F), beats 0..16
    {
        ...
        auto pattern = chords.addPattern (slotA1, "Am F", 8.0, nullptr);
    ...
    // Lead: 8-beat melody, enters at beat 8
    {
        ...
        auto pattern = lead.addPattern (slotA1, "Melody", 8.0, nullptr);
